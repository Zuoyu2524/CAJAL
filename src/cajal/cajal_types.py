...
Distribution: TypeAlias = npt.NDArray[np.float32]
# A DistanceMatrix is a square symmetric matrix with zeros along the diagonal
# and nonnegative entries.
DistanceMatrix: TypeAlias = npt.NDArray[np.float32]
Matrix = NewType("Matrix", npt.NDArray[np.float32])
# An Array is a one-dimensional matrix.
Array = NewType("Array", npt.NDArray[np.float32])
